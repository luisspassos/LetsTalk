...
type FormattingObj = Record<string, string>;
...
const formatTextToHtml = (text: string, extraFormatting?: FormattingObj) => {
  // for security reasons

  const formatting: FormattingObj = {
    '&': '&amp;',
    '"': '&quot;',
    "'": '&#39;',
    '<': '&lt;',
  ...
  for (const prop in formatting) {
    const result = formatting[prop];

    text = text.replaceAll(prop, result);
  }
...
export function MessageInput() {
  ...
  useEffect(() => {
    ...
    const events: Events = [
      ...
      {
        type: 'beforeinput',
        func: handleValuesWithEmojisFromInputMethodEditor,
      },
      {
        type: 'input',
