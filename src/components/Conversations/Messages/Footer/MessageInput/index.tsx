...
const getTextFormattedToHtml = (el: Element) => {
    ...
    async function handleValuesWithoutEmojisAndDeletion(e: InputEvent) {
      ...
      if (textDirectionHasBeenChanged) {
        const changeDirection = () => {
          ...
          const textFormatted = getTextFormattedToHtml();

          // const twemojis = messageInput?.matches;

          // let twemojiIndex = 0;

          // const message = text?.replace(emojiRegex, () => {
          //   const twemoji = twemojis?.item(twemojiIndex).outerHTML;
